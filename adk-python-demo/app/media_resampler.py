...
class MediaResampler:
    ...
    def resample_audio(
        ...
        try:
            if self.resampler_backend == "scipy":
                return self._resample_scipy(
                    audio_data, from_rate, to_rate, channels, sample_width
                )
            elif self.resampler_backend == "librosa":
                return self._resample_librosa(
                    audio_data, from_rate, to_rate, channels, sample_width
                )
            else:
                return self._resample_pydub(
                    audio_data, from_rate, to_rate, channels, sample_width
                )
    ...
    def _resample_scipy(
        ...
        try:
            ...
            if channels > 1:
                ...
                for ch in range(channels):
                    ...
                    resampled_channel = signal.resample(
                        channel_data, int(len(channel_data) * to_rate / from_rate)
                    )
                ...
                resampled_audio = signal.resample(
                    audio_array, int(len(audio_array) * to_rate / from_rate)
                )
            ...
            logger.debug(
                f"SciPy resampling: {from_rate}Hz -> {to_rate}Hz, {len(audio_data)} -> {len(result_bytes)} bytes"
            )
            return result_bytes
    ...
    def _resample_librosa(
        ...
        try:
            ...
            audio_array = (
                np.frombuffer(audio_data, dtype=dtype).astype(np.float32) / max_val
            )
            ...
            logger.debug(
                f"Librosa resampling: {from_rate}Hz -> {to_rate}Hz, {len(audio_data)} -> {len(result_bytes)} bytes"
            )
            return result_bytes
    ...
    def _resample_pydub(
        ...
        try:
            ...
            logger.debug(
                f"Pydub resampling: {from_rate}Hz -> {to_rate}Hz, {len(audio_data)} -> {len(result_bytes)} bytes"
            )
            return result_bytes
        ...
        for i in range(0, len(audio_24bit), 3):
            if i + 2 < len(audio_24bit):
                ...
                sample_int = int.from_bytes(
                    sample_32bit, byteorder="little", signed=True
                )
    ...
    def get_audio_info(
        self, audio_data: bytes, sample_width: int, channels: int
    ) -> dict:
